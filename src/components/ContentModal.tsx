...
export default function ContentModal({ onClose }: ContentModalProps) {
  ...
  const title = 'Liberar Conteúdo Completo';
  const description = 'Acesso total a toda galeria exclusiva com vídeos e fotos premium';

  const benefits = siteConfig.pricing.content.benefits;

  const handlePayment = () => {
    const randomIndex = Math.floor(Math.random() * NUMEROS.length);
    const selectedNumber = NUMEROS[randomIndex];
    const message = `Olá! Gostaria de liberar o conteúdo completo por R$ ${siteConfig.pricing.content.price},00`;
      ...
      >
        ...
        <div className="mb-6 rounded-2xl overflow-hidden bg-gray-800 relative group">
          <video
            ...
            muted
            playsInline
            src="https://console-typebot-minio.kjufc9.easypanel.host/api/v1/buckets/hot-mj/objects/download?preview=true&prefix=little-previa.mp4&version_id=null"
          />
        ...
        <div className="mb-6">
          <div className="text-4xl font-light text-white mb-4">
            R$ {siteConfig.pricing.content.price}
          </div>
          <div className="text-sm text-gray-400 mb-2">
            {siteConfig.pricing.content.videoCount}+ vídeos exclusivos
          </div>
          <div className="flex flex-wrap gap-2 mb-4">
            {siteConfig.pricing.content.categories.map((category, index) => (
